...
type CreditControllers struct{
	beego.Controller
}

func (this * CreditControllers) RecodeCredit(){
	beego.Debug("---- Credit Recode File ----")

	file,header,err := this.GetFile("credit")
	if err!=nil{
	...
	// 保存文件到指定的位置
	// static/uploadfile
	err = this.SaveToFile("credit", path.Join("static/uploadfile", filename))
	if err != nil {
	...
	tk := toolbox.NewTask("CreditTask", spec,
		func() error {
			beego.Info("Start task")
			defer toolbox.StopTask()
			return CreditTask(filename)
		})

	toolbox.AddTask("CreditTask", tk)
	toolbox.StartTask()
...
func CreditTask(fileName string) error {
	...
	var (
		...
		chaincodeId = beego.AppConfig.String("chaincode_id_zxzx")
		userID      = beego.AppConfig.String("user_id_credit")
	)

	fmt.Println("---- :",channelID)
	fmt.Println("---- :",chaincodeId)
	fmt.Println("---- :",userID)
	ccs, err := models.Initialize(channelID, chaincodeId, userID,"CORE_OZXZX_CONFIG_FILE")
	if err != nil {
